---
name: how
description: "Use for \"how does X work\", code walkthroughs before changing something, and placement / ownership / layering questions (\"where should this live\", \"which package owns this\", \"is this the right layer\"). Explains subsystem architecture, runtime flow, onboarding mental models, and architectural concerns."
---
...
2. **Critique.** Explain first, then ask independent reviewers to identify architectural issues
...
Launch all explorers as fresh-context, read-only `scout` subagents in one parallel workflow. Let the configured agent routing choose their models.
...
- Start broad: find relevant directories and files, then search for key types, interfaces, and class names
...
Launch a single fresh-context, read-only `scout` subagent that explores and explains in one pass. Let the configured agent routing choose its model.

This agent does its own file discovery, code search, and reading, then writes the explanation directly. Read `references/explainer-prompt.md` for the communication style and output format. The agent follows the same structure, it just doesn't have explorer findings as input.

Proceed to Step 4.

### Step 3. Synthesize (complex questions only)

Once all explorers have returned, launch a single fresh-context, read-only `delegate` subagent to synthesize their findings into one coherent explanation. Let the configured agent routing choose its model.
...
After the explanation is complete, launch three fresh-context, read-only `reviewer` subagents in one parallel workflow. Give them distinct lenses so they do not duplicate work:

- Boundaries and data model
- Abstraction fit and complexity
- Evolution readiness and consistency

Let the configured agent routing choose their models.
