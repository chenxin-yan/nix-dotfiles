...
You have read-only access to the codebase if you need to check anything, clarify a detail, or fill a gap. Use the available file discovery, code search, and read tools as needed. But the explorers already did the heavy lifting, so you shouldn't need to re-explore from scratch.
